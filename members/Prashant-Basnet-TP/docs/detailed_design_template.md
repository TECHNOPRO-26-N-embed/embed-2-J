...
|:---|:---             |:---                        |:---         |:---       |:---|
| 1  | loop()          | ボタンを1回押す              | サイコロが1回だけ振られる | 1回の押下で1回のみ実行を確認 | Pass |
| 2  | loop()          | ボタンを連打                 | 1回ごとに1回だけ振られる | 表示中の追加入力は無視され、誤作動なし | Pass |
| 3  | loop()          | 前回押下から199msで再押下     | 再実行されない（デバウンスで無効） | 199ms以内の再押下は無効化された | Pass |
| 4  | loop()          | 前回押下から200ms以上で再押下 | 再実行される | 200ms以上経過後の再押下で再実行された | Pass |
